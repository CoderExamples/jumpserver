...
class SerializeApplicationToTreeNodeMixin:
    ...
    @staticmethod
    def create_system_user_node(tree_id, app_id, parent_info, user):
        tree_nodes = []
        ...
        for system_user in system_users:
            tree_nodes.append(
                Application.as_k8s_system_tree_node(
                    tree_id, system_user, parent_info
                )
            )
        return tree_nodes
    ...
    def serialize_applications_with_org(self, applications, tree_id, parent_info, user):
        ...
        if not tree_id:
            ...
            pod_name = parent_info.get('pod_name')
            if pod_name:
                ...
                for container in next(
                    ...
                    tree_nodes.append(
                        Application.as_k8s_namespace_pod_tree_node(
                            tree_id, container, 0, 'container', parent_info, is_container=True
                        )
                    )
            elif namespace:
                data = self.get_kubernetes_data(app_id, system_user_id)
                ...
                for pod in data[namespace]:
                    ...
                    tree_nodes.append(
                        Application.as_k8s_namespace_pod_tree_node(
                            tree_id, pod['pod_name'], counts, 'pod_name', parent_info
                        )
                    )
            elif system_user_id:
                data = self.get_kubernetes_data(app_id, system_user_id)
                ...
                for namespace, pods in data.items():
                    ...
                    tree_nodes.append(
                        Application.as_k8s_namespace_pod_tree_node(
                            tree_id, namespace, counts, 'namespace', parent_info
                        )
                    )
            elif app_id:
                tree_nodes += self.create_system_user_node(tree_id, app_id, parent_info, user)
        return tree_nodes
