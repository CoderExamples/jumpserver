...
class ApplicationTreeNodeMixin:
    ...
    @staticmethod
    def create_kubernetes_tree_node(id_, pid, name, identity, parent_info, icon='', is_container=False):
        node = TreeNode(**{
            'id': id_,
            'name': name,
            'title': name,
    ...
    def as_k8s_tree_node(self, pid):
        ...
        node = self.create_kubernetes_tree_node(
            app_id, pid, self.name, 'k8s', parent_info
        )
        return node

    @classmethod
    def as_k8s_system_tree_node(cls, pid, system_user, parent_info):
        system_user_id = str(system_user.id)
        username = system_user.username
        username = username if username else '*'
        name = f'{system_user.name}({username})'
        pid = urlencode({'app_id': pid})
    ...
    @classmethod
    def as_k8s_namespace_pod_tree_node(cls, pid, name, counts, type, meta, is_container=False):
        i = cls.create_tree_id(pid, type, name)
        meta.update({type: name})
        name = name if is_container else f'{name}({counts})'
        icon = 'cloud'
        node = cls.create_kubernetes_tree_node(
            i, pid, name, type, meta, icon=icon, is_container=is_container
        )
        return node
